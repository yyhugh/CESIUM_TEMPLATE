<template>
  <div class="v-cesium">
    ...
    <TextTooltip :show="pointTextTooltip" :offsetY="-40" ref="pointTextTooltipRef"></TextTooltip>
    <TextTooltip :show="planeTextTooltip" :offsetY="-45" ref="planeTextTooltipRef"></TextTooltip>
  </div>
</template>
...
const pointTextTooltipRef = ref();
const pointTextTooltip = ref(false);
const planeTextTooltipRef = ref();
const planeTextTooltip = ref(false);
let pointHandler: () => void;
let planeHandler: () => void;

watch(pointTextTooltip, (next) => {
  if (!next && viewerIns && pointHandler) {
    viewerIns.scene.postRender.removeEventListener(pointHandler);
  }
});

watch(planeTextTooltip, (next) => {
  if (!next && viewerIns && planeHandler) {
    viewerIns.scene.postRender.removeEventListener(planeHandler);
  }
});
...
function init() {
  ...
  CesiumUtil.on("click", viewer, (event) => {
    pointTextTooltip.value = false;
    planeTextTooltip.value = false;
    ...
    if (Cesium.defined(pick)) {
      ...
      if (pick.id.id === "点") {
        ...
        pointTextTooltipRef.value.setPosition(winPos);
        pointTextTooltip.value = true;
        pointHandler = () => {
          if (pointTextTooltip.value) {
            const winPos = viewer.scene.cartesianToCanvasCoordinates(pick.id.position._value);
            pointTextTooltipRef.value.setPosition(winPos);
          }
        };
        viewer.scene.postRender.addEventListener(pointHandler);
      } else if (pick.id.id === "面") {
        const winPos = viewer.scene.cartesianToCanvasCoordinates(pick.id.position._value);
        planeTextTooltipRef.value.setPosition(winPos);
        planeTextTooltip.value = true;
        planeHandler = () => {
          if (planeTextTooltip.value) {
            const winPos = viewer.scene.cartesianToCanvasCoordinates(pick.id.position._value);
            planeTextTooltipRef.value.setPosition(winPos);
          }
        };
        viewer.scene.postRender.addEventListener(planeHandler);
      }
    }
  });
